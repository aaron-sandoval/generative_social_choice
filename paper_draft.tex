...
\begin{itemize}
    \item \textbf{LLM embeddings.} We embed each opinion summary with OpenAI's \texttt{text-embedding-3-small} and measure cosine similarity.
    \item \textbf{Discriminative distances \citep{fish-v2}} We ask voter \(j\) to rate the summary statement of voter \(i\).
\end{itemize}
...
\begin{description}
    \item[Single-statement generator] Generate one statement intended to represent the set of opinions; we run this generator twice.
    \item[Batch generator] Generate five statements at once, prompted to jointly represent the set of opinions as well as possible.
\end{description}
...
evaluate them against a set of social choice axioms as well as the social welfare functions in \Cref{sec:metrics}.

\subsection{Algorithms}

We compare three assignment-based utility-maximization methods with and without a  geometric utility transformation that biases toward egalitarian outcomes. Since \citet{fish-v2} does not include a single step in which a voting algorithm is applied, we include reweighted range voting \citep{rangevoting-rrv} as a simple baseline.
...
RRV is a greedy multiwinner range voting algorithm. All ballots initially have equal weight \(q_i=1\), and the highest-scoring candidate wins. After each selection, weights are updated according to
\[
q_i = \frac{K}{K + \frac{1}{u_{\max}}\sum_{j\in W_{\mathrm{partial}}} u_{ij}},
\]
where \(W_{\mathrm{partial}}\) is the set of already selected winners, \(u_{\max}\) is the maximum ballot utility, and \(K\in[0.5,1]\) is a hyperparameter.
...
\begin{description}
    \item[Symmetry] Changing the order of voters or statements in the ballot profile $U$ does not change $\omega$.
    ...
    \item[Individual Pareto Efficiency] No $\omega'$ increases total utility without decreasing any individual voter's utility.
    \item[Sorted Pareto Efficiency] \textbf{\citep{sorted-pareto}:} No $\omega'$ has a sorted utility vector that Pareto-dominates that of $\omega$.
    \item[Generalized Lorenz Efficiency] \textbf{\citep{shorrocks-1983}:} No $\omega'$ has a dominating generalized Lorenz curve. Equivalently, let $u_{\omega}$ be the voter utility vector on the selected slate, and let $u_{\omega'}$ be the voter utility vector for some $\omega'$. When all voters are anonymized, it is impossible to select an $\omega'$ such that the anonymous transformation of $u_{\omega}$ to $u_{\omega'}$ can be expressed as a non-empty series of actions either granting positive utility to a voter or transferring utility from "wealthier" voters to "poorer" voters.
    \item[Egalitarian--Utilitarian Efficiency] No $\omega'$ has both greater total and minimum utility.
    \item[Non-radical Utilitarianism] No $\omega'$ has marginally lower total utility and substantially higher minimum utility. More formally, with the selected slate $\omega$ having total utility $u_{\mathrm{tot}}$ and minimum utility $u_{\mathrm{min}}$ there exists no $\omega'$ with total utility $u_{tot} - \delta$ and minimum utility $u_{\mathrm{min}} + \epsilon$, where $\epsilon > 0, \delta > 0$ and $\epsilon/\delta$ is arbitrarily large.
\end{description}

Generalized Lorenz Efficiency implies Sorted Pareto Efficiency, which in turn implies Individual Pareto Efficiency. 
...
We evaluate voting rules against each axiom using a set of manually constructed adversarial ballot profiles. For cases where the voting rules pass all adversarial cases, we additionally look for counterexamples  manually and using LLMs (ChatGPT 5.3 and Opus 4.7). Nevertheless, as we don't provide formal proofs of satisfaction, false positives remain possible. Results are shown in \Cref{tab:voting-axioms}. %These are best-effort empirical checks rather than formal proofs of satisfaction, so false positives remain possible.
...
\begin{figure*}[t]
\centering
  \includegraphics[width=0.8\textwidth]{figures/voting_alg_scalar_metrics_with-Monroe.png}
  \caption{Confidence intervals for mean voting algorithm performance on social welfare functions. Confidence intervals are bootstrapped across 20 ballot profiles generated from the chatbot personalization dataset. All algorithms select a slate of 3 candidates.}
  \label{fig:sfws-voting-algs}
\end{figure*}

We also evaluate each voting rule on the social welfare functions from \Cref{sec:metrics}. Using the 20 ballot profiles generated from our pipeline in the embedding ablation study (Appendix~\ref{app:embedding-ablation-details}), we produce the results in Figure~\ref{fig:sfws-voting-algs}. Exact with a mild geometric transformation (\(p=1.5\)) behaves almost identically to the untransformed rule on this benchmark, so in the figure we additionally show a more aggressive transformation (\(p=6.0\)) for contrast. Exact, Greedy, and LP perform similarly. RRV performs poorly under our assignment-based utility because it rewards candidates with broad appeal across the whole slate rather than candidates that serve as especially good representatives for particular voters. When evaluating the resulting slates under RRV's whole slate utility model, RRV dominates.
...
AS produced the initial idea for the project, and both authors refined it. AS performed the literature review and contextualization of the project, as well as the selection, design, and testing of the axioms. PB developed and implemented all of the voting algorithms except for reweighted range voting, did initial design of some axioms, implemented the LLM embeddings, and ran the embedding and statement count ablation studies. Both authors performed main pipeline experiments, analysis, time complexity analysis, and both wrote the paper.
...
\section{Proofs of Axiom Satisfaction}
\label{app:axiom-proofs}
We prove every axiom marked \Pass{} in \Cref{tab:voting-axioms}. Throughout, $U\in\mathbb{R}_{\ge 0}^{n\times m}$ is the ballot profile, a solution is a pair $(W,\omega)$ with $W\subseteq[m]$, $|W|=k$, and $\omega:N\to W$, and voter $i$'s realized utility is $U_{i,\omega(i)}$. As in \Cref{sec:axioms}, we work under the genericity assumption that no two distinct slates (and, for the greedy rules, no two candidates at any step) attain exactly equal objective values, so that each algorithm's output is unique; this is the same tie caveat used to define axiom failure.

For the utility-maximizing rules it is convenient to write, for a slate $W$,
\[
T(W)=\sum_i \max_{j\in W} U_{ij},
\]
\[
F(W)=\sum_i \max_{j\in W} f(U_{ij})=\sum_i f\!\Big(\max_{j\in W} U_{ij}\Big),
\]
where $f(u)=\frac{1-(1/p)^u}{1-1/p}$ is strictly increasing and, for $p>1$, strictly concave. The optimal assignment for a fixed slate sends each voter to a most-preferred member of the slate; Exact maximizes $T$, Exact$(p>1)$ maximizes $F$.
...
\paragraph{Symmetry.} $T$ is built from sums and maxima of utility entries and is invariant under relabeling voters or candidates. Permuting the rows or columns of $U$ therefore permutes the optimal slate and assignment identically, leaving $\omega$ unchanged as a map.

\paragraph{Monotonicity.} \emph{(Winner.)} Let $c\in W^\star$ and raise $U_{ic}$ by $\Delta>0$, giving $U'$. For every slate $W\not\ni c$ the changed entry is unused, so $T'(W)=T(W)$; for $W^\star\ni c$, $T'(W^\star)\ge T(W^\star)$. Hence for every $c$-free slate $W'$,
\[
T'(W^\star)\ge T(W^\star)>T(W')=T'(W'),
\]
so $W^\star$ still beats every slate omitting $c$, and any slate beating $W^\star$ contains $c$. The new optimum therefore contains $c$. \emph{(Loser.)} Let $c\notin W^\star$ and lower $U_{ic}$. Then $T'(W^\star)=T(W^\star)$, while every $W'\ni c$ has $T'(W')\le T(W')<T(W^\star)$, so the optimum still omits $c$.

\paragraph{Individual Pareto Efficiency.} Exact attains the maximum total utility $T^\star$ over all feasible solutions, so no $\omega'$ has strictly greater total utility; a fortiori none raises total utility without lowering some voter.

\paragraph{Sorted Pareto Efficiency.} Sorting permutes a vector and preserves its sum. If some $\omega'$ had a sorted utility vector Pareto-dominating that of $\omega$, its sum—hence its total utility—would exceed $T^\star$, contradicting maximality.

\paragraph{Generalized Lorenz Efficiency.} The final ordinate of the generalized Lorenz curve equals total utility. If $\omega'$ generalized-Lorenz-dominates $\omega$, its final ordinate is $\ge T^\star$. If strictly greater, then $T(\omega')>T^\star$, impossible; if equal, then $T(\omega')=T^\star$, so $\omega'$ is also total-optimal, a tie excluded by genericity. Hence no $\omega'$ dominates.

\paragraph{Egalitarian--Utilitarian Efficiency.} Since no $\omega'$ exceeds $T^\star$ in total utility, none has both greater total and greater minimum utility.

\subsection{Exact$(p>1)$}
Exact$(p>1)$ is Exact applied to the transformed utilities $f(U_{ij})$, i.e.\ it maximizes $F$.

\paragraph{Symmetry.} $F$ is invariant under relabeling, exactly as for Exact.

\paragraph{Monotonicity.} Raising $U_{ic}$ raises $f(U_{ic})$ since $f$ is strictly increasing, so $F'(W)\ge F(W)$ for $W\ni c$ and $F'(W)=F(W)$ for $W\not\ni c$; the winner argument for Exact applies verbatim to $F$. Lowering a loser's $U_{ic}$ lowers $f(U_{ic})$, and the loser argument applies likewise.

\paragraph{Individual Pareto Efficiency.} Suppose $\omega'$ raises total raw utility without lowering any voter's raw utility. Then $U_{i,\omega'(i)}\ge U_{i,\omega(i)}$ for all $i$, with strict inequality for at least one $i$. Since $f$ is strictly increasing, $f(U_{i,\omega'(i)})\ge f(U_{i,\omega(i)})$ for all $i$ with at least one strict inequality, so $F(\omega')>F(\omega)$, contradicting optimality of $\omega$.

\paragraph{Sorted Pareto Efficiency.} Because $f$ is applied pointwise and sums are permutation-invariant, $F(\omega)=\sum_\ell f(s_\ell)$, where $s$ is the sorted raw utility vector. If $\omega'$ sorted-dominates $\omega$, then $s'_\ell\ge s_\ell$ for all $\ell$ with one strict inequality, so $F(\omega')=\sum_\ell f(s'_\ell)>\sum_\ell f(s_\ell)=F(\omega)$: contradiction.

\paragraph{Generalized Lorenz Efficiency.} By the Hardy--Littlewood--P\'olya / Shorrocks characterization \citep{shorrocks-1983}, $x$ generalized-Lorenz-dominates $y$ iff $\sum_i g(x_i)\ge\sum_i g(y_i)$ for every increasing concave $g$. Strict dominance can be written as a finite sequence of nonnegative increments and Pigou--Dalton progressive transfers, at least one of them nontrivial; each strictly increases $\sum_i f(\cdot)$ because $f$ is strictly increasing and strictly concave. Hence a feasible $\omega'$ dominating $\omega$ would satisfy $F(\omega')>F(\omega)$, contradicting optimality.

\subsection{Greedy}
Greedy builds $W$ by repeatedly adding the candidate of largest marginal gain
\[
\Delta(x\mid S)=\sum_i\big[\max(U_{ix},M_i(S))-M_i(S)\big],
\]
\[
M_i(S)=\max_{j\in S}U_{ij},
\]
and assigns each voter to a most-preferred member of the final slate.

\paragraph{Independence of Common Scale.} Scaling $U$ by $c>0$ scales every $\Delta(x\mid S)$ by $c$, preserving each greedy choice; positive scaling preserves the assignment preferences.

\paragraph{Symmetry.} Each marginal gain depends only on utility values, not labels, so relabeling permutes the selected sequence identically.

\paragraph{Monotonicity.} The key fact is that for any partial slate $S$ with $c\notin S$, the marginal gain $\Delta(x\mid S)$ of every candidate $x\ne c$ is independent of $U_{ic}$. \emph{(Winner.)} Raise $U_{ic}$ for $c\in W$; then $\Delta(c\mid S)$ is nondecreasing in $U_{ic}$ for all $S\not\ni c$, while all other gains are unchanged. Run greedy on $U$ and on the modified $U'$ in lockstep: until $c$ is chosen, the partial slate contains no $c$, so each non-$c$ candidate has identical gain in both runs and $c$'s gain is weakly larger under $U'$. At each step either the same non-$c$ candidate is chosen as under $U$, or $c$ overtakes and is chosen earlier; at the step where $U$ chose $c$, its $U'$-gain is at least its (maximal) $U$-gain, so $c$ is chosen. In every case $c\in W'$. \emph{(Loser.)} Lowering $U_{ic}$ for $c\notin W$ only decreases $c$'s gains and leaves all others unchanged, so the lockstep run is identical and never selects $c$.

\subsection{LP}
LP solves the relaxation, selects the $k$ candidates with largest $x_j$, and assigns each voter to a most-preferred selected candidate.

\paragraph{Independence of Common Scale.} Scaling $U$ by $c>0$ scales the linear objective by $c$ while leaving the feasible polytope unchanged, so the set of optima—and, for a deterministic solver, the returned vertex—is unchanged. The top-$k$ selection and the preference-based assignment are likewise invariant under positive scaling.

\subsection{RRV}
RRV greedily selects $\arg\max_j \sum_i q_i U_{ij}$ with
$q_i = K/\big(K+\tfrac{1}{u_{\max}}\sum_{j\in W_{\mathrm{partial}}} U_{ij}\big)$,
where $u_{\max}$ is the fixed maximum of the rating scale, a constant independent of the profile. (Treating $u_{\max}$ as fixed rather than profile-dependent is precisely why RRV fails Independence of Common Scale; see \Cref{tab:failure-rrv-scale}.)

\paragraph{Symmetry.} $u_{\max}$ is a constant and the weighted scores depend only on utility values, so relabeling voters or candidates permutes the selection identically.

\paragraph{Monotonicity.} Because $u_{\max}$ is constant, the weights $q_i$ depend on the profile only through the utilities of already-selected winners. Hence for any partial slate with $c\notin W_{\mathrm{partial}}$, neither the weights nor the weighted score of any candidate $x\ne c$ depends on $U_{ic}$, while $c$'s own weighted score $\sum_i q_i U_{ic}$ is nondecreasing in $U_{ic}$. The lockstep argument from the Greedy proof then applies verbatim: raising a winner's rating makes $c$ be selected at the same step or earlier, and lowering a loser's rating leaves the run unchanged.


\section{Reasoning and Counterexample Ballot Profiles for Axiom Failures}
\label{app:voting-failures}

\Cref{tab:failure-exact-scale,tab:failure-exact-nru,tab:failure-exactp-nru,tab:failure-rrv-scale,tab:failure-greedy-indiv-pareto,tab:lp-fractional,tab:lp-monotonicity}
...
\hline
\end{tabular}
\caption{RRV fails Independence of Common Scale for $k=2$, \(K=1\), \(u_{\max}=1\), and scale factor \(0.5\). The resulting slate changes from \(\{s_1,s_2\}\) to \(\{s_1,s_3\}\) after scaling.}
...
\hline
\end{tabular}
\caption{Greedy Total Utility Maximization and RRV fail Individual Pareto Efficiency for $k=2$. The selected slates include \(s_1\), while the only slate satisfying the axiom is \(\{s_2,s_3\}\).}
...
\subsection{Exact$(p>1)$ fails Non-radical Utilitarianism}
\label{app:exactp-nru}
% Example found by Claude Opus 4.7

The geometric transformation does not repair this failure. Exact$(p>1)$
...
\hline
\end{tabular}
\caption{Counterexample showing that Exact$(p>1)$ fails Non-radical
...
\]
Exact$(p>1)$ selects the slate with the larger transformed total. For \(s_1\),
\[
...
\]
The bound holds for every \(\delta\ge 0\), so Exact$(p>1)$ selects \(s_1\).
...
Exact$(p>1)$ violates Non-radical Utilitarianism.

The same profile is a counterexample for every \(p>1\), hence Exact$(p>1)$ fails Non-radical Utilitarianism for all \(p>1\).
