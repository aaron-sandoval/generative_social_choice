...
We do not aim to satisfy committee monotonicity \citep{elkind-committee-monotonicity} or balanced justified representation \citep{fish-v2}. \Cref{tab:committee-monotonicity-example,tab:bjr-example} illustrate why these properties may be undesirable for opinion condensation.
...
We evaluate voting rules against each axiom using a set of adversarial ballot profiles. These profiles are generated both manually and using LLMs (ChatGPT 5.3 and Claude Opus 4.7).
For \Exact{}, we provide formal proofs of satisfaction of some axioms in \Cref{app:exact-axiom-proofs}.
...
\paragraph{Virtual voters use weak LLMs}
...
\paragraph{Non-radical Utilitarianism}
This axiom is an attempt to make a minimally restrictive illustration of a limitation of a pure utilitarian objective function. However, the choice of minimum utility as the measure of egalitarianism imposes a subjective judgment incompatible with other measures of egalitarianism \citep{atkinson-1970}. There are ballot profiles, such as the example in Table~\ref{tab:egal-util-strict}, where an assignment that violates the axiom may yet be reasonably preferred on egalitarian grounds. Since no scalar-valued social welfare function can fully capture the idea of fairness, this axiom's reliance on minimum utility as an egalitarian measure is overly constraining.
...
\Cref{tab:failure-exact-scale,tab:failure-exact-nru,tab:failure-exactp-nru,tab:failure-greedy-indiv-pareto,tab:lp-fractional,tab:lp-monotonicity,tab:failure-monroe-egal-util,tab:failure-monroe-nru}
...
\subsection{\Monroe{} fails Egalitarian--Utilitarian Efficiency}
\label{app:monroe-egal-util}

All three \Monroe{} variants share the greedy selection procedure of
\Monroe(equal, greedy) described in \Cref{sec:voting-algorithms}. \Cref{tab:failure-monroe-egal-util} demonstrates a failure to select an efficient slate, producing a failing voter assignment for all \Monroe{} variants.
...
\hline
\end{tabular}
\caption{Profile with $k=2$ for which all three \Monroe{} variants fail
Egalitarian--Utilitarian Efficiency. Every variant selects $\{s_2,s_3\}$, which
is dominated on both total and minimum utility by $\{s_1,s_2\}$.}
\label{tab:failure-monroe-egal-util}
\end{table}

\subsection{\Monroe{} fails Non-radical Utilitarianism}
\label{app:monroe-nru}

\Cref{tab:failure-monroe-nru} gives a profile with \(k=2\), parameterized by a
free parameter \(\delta>0\), for which all three \Monroe{} variants sacrifice an
arbitrarily large amount of minimum utility to gain an arbitrarily small amount
of total utility.
...
$u_2$ & $8$   & $0.3$ & $0.4$ & $6.2$ \\
$u_3$ & $0.5$ & $6$   & $5.8$ & $0.6$ \\
$u_4$ & $0.7$ & $1$   & $0.8$ & $5-\delta$ \\
\hline
\end{tabular}
\caption{Profile with $k=2$ for which all three \Monroe{} variants fail
Non-radical Utilitarianism, for any sufficiently small \(\delta>0\).}
\label{tab:failure-monroe-nru}
\end{table}

Given the ballot profile in \Cref{tab:failure-monroe-nru}, all \Monroe{} variants select \(s_1, s_2\) with utility vector
\((9,8,6,1)\). This results in 
\[
u_{\mathrm{tot}}=24,\qquad u_{\mathrm{min}}=1 .
\]

The slate \(\{s_3,s_4\}\) gives \((7,6.2,5.8,5-\delta)\), so
\[
u_{\mathrm{tot}}=24-\delta,\qquad u_{\mathrm{min}}=5-\delta .
\]
Moving to it sacrifices \(\delta\) in total utility while improving the minimum
utility by \(\epsilon=4-\delta\). The ratio
\[
\frac{\epsilon}{\delta}=\frac{4-\delta}{\delta}
\]
is arbitrarily large by taking \(\delta\) arbitrarily small, so all
three \Monroe{} variants violate Non-radical Utilitarianism.
