...
\paragraph{Monroe \citep{skowron-2015}.}
The Monroe rule selects a slate together with an assignment of an equal-sized constituency of voters to each winner. We adapt the stepwise greedy procedure used by \citet{fish-v2} to operate on the complete ballot profile. The slate is built in \(k\) rounds. In each round, every remaining candidate is scored by the \(\lceil n/k\rceil\)-th highest utility among the unassigned voters. The highest-scoring candidate is added to the slate, and those top \(\lceil n/k\rceil\) unassigned voters are provisionally assigned to it.

\paragraph{Monroe (equal, greedy).}
Voters keep the equal-sized constituency assignment produced during the greedy selection. Identical to \citet{fish-v2}, the baseline.

\paragraph{Monroe (equal, optimal).}
After the provisional assignmen, the final assignment is recomputed to maximize the Monroe score: the total utility attainable when each selected statement represents an equal-sized constituency. We solve this assignment with an integer linear program. Because the constraint matrix of this fixed-slate balanced-assignment problem is totally unimodular, the program solves the assignment exactly in polynomial time. The slate itself is still chosen greedily, so the overall procedure is not guaranteed to find a globally optimal Monroe slate.

\paragraph{Monroe (free, greedy).}
The equal-sized-constituency constraint is dropped and each voter is assigned to their favorite member of the final slate.
...
\hline
 & Exact & Exact($p>1$) & Greedy & LP & \makecell{Monroe\\(eq, greedy)} & \makecell{Monroe\\(eq, opt)} & \makecell{Monroe\\(free, greedy)} \\
\hline
Time Complexity
...
& $O(kmn)$
& \makecell{$O(kmn)$\\$+$ assignment ILP\tablefootnote{The greedy selection is $O(kmn)$; the final balanced-constituency assignment is solved by an integer program whose constraint matrix is totally unimodular, so it is solved exactly in polynomial time.}}
& $O(kmn)$ \\
...
% TODO(author): add discussion of the Stepwise Monroe variants' performance here.
...
\subsection{Stepwise Monroe}
All three Stepwise Monroe variants select the same slate by the greedy procedure of
